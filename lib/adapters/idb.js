...
function init(api, opts, callback) {
  ...
  api._changes = function (opts) {
    ...
    var allDocsStyle = opts.style === 'all_docs';
    ...
    function onsuccess(event) {
      ...
      if (allDocsStyle && emittedDocIds.has(doc._id)) {
        // don't emit twice
        return cursor.continue();
      }
      ...
      function onFetchMetadata(metadata) {
        ...
        // In the case of normal changes(), skip non-winning revs
        // In the case of style=all_docs, include all revs
        if (allDocsStyle) {
        ...
        } else { // normal style, only care about the winner
          if (winningRev === doc._rev) {
            onGetWinningDoc(doc, metadata);
          } else {
            return cursor.continue();
